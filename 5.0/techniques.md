...
passport 是目前最流行的 node.js 认证库，为社区所熟知，并相继应用于许多生产应用中。使用专用 passport 实用程序将此工具与 Nest 框架集成起来非常简单。为了演示，我们将设置 passport-http-bearer  和 passport-jwt 策略。
...
validateUser ( )方法将令牌作为参数。此令牌从与HTTP请求一起传递的授权标头中提取。findOneByToken ( )方法的职责是验证传递的令牌是否确实存在，并与数据库中的任何注册帐户关联。
