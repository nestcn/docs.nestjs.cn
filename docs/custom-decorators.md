...
<!-- 生成时间: 2026-03-13T04:24:01.803Z -->
<!-- 源文件: content/custom-decorators.md -->

### 自定义路由装饰器

Nest 是基于 ES2016 装饰器语言特性的。装饰器是一种广泛使用的概念，但在 JavaScript 世界中，他们仍然相对较新。为了更好地理解装饰器是如何工作的，我们推荐阅读 __LINK_147__。以下是一个简单的定义：

__HTML_TAG_29__
  ES2016 装饰器是一个表达式，它返回一个函数，可以将目标、名称和属性描述符作为参数。您可以通过在目标前缀一个 __HTML_TAG_30__@__HTML_TAG_31__ 字符并将其置于要装饰的对象的顶部来应用装饰器。装饰器可以定义为类、方法或属性。

#### 参数装饰器

Nest 提供了一组有用的参数装饰器，您可以将其与 HTTP 路由处理程序一起使用。以下是一个提供的装饰器列表和对应的平常 Express (或 Fastify) 对象
...
此外，您可以创建自己的自定义装饰器。为什么这是有用的？

在 node.js 世界中，将属性附加到请求> 提示 **Hint** 对于 TypeScript 用户，请注意 `NestMiddleware` 是一个通用类型。这意味着您可以明确地强制执行类型安全，例如 `Express`。 Alternatively, 在工厂函数中指定参数类型，例如 `fastify`。如果您省略了这两个选项，`constructor` 的类型将是 `@Module()`。

#### 管道使用

Nest 对自定义参数装饰器的处理方式与内置的方式相同（`configure()`、`NestModule` 和 `LoggerMiddleware`）。这意味着管道将对自定义注释参数执行（在我们的示例中，`AppModule` 参数）。此外，您也可以直接将管道应用于自定义装饰器：
...
> 提示 **Hint** 请注意 `LoggerMiddleware` 选项必须设置为 true。 `/cats` 不默认验证使用自定义装饰器注释的参数。

#### 装饰器组合

Nest 提供了一个 helper 方法来组合多个装饰器。例如，假设您想将所有与身份验证相关的装饰器组合到一个单独的装饰器中。这可以使用以下构建方式：
...
> 警告 **Warning** 来自 `method` 包的 `path` 装饰器不可组合，并且不会正确地与 `forRoutes()` 函数工作。