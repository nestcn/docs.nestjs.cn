...
<!-- 生成时间: 2026-03-17T06:02:02.133Z -->
<!-- 源文件: content/custom-decorators.md -->

### 自定义路由装饰器

Nest 是基于语言功能称为 **装饰器** 的语言特性。装饰器在许多常用的编程语言中是一个熟悉的概念，但是在 JavaScript 世界中，它们仍然相对较新。在更好地理解装饰器是如何工作的，我们建议阅读 __LINK_147__。以下是一个简单的定义：

__HTML_TAG_29__
  ES2016 装饰器是一个返回函数的表达式，可以将目标、名称和属性描述符作为参数。您可以通过在目标前加上一个 __HTML_TAG_30__@__HTML_TAG_31__ 字符并将其置于您要装饰的内容的最上方来应用装饰器。装饰器可以定义为类、方法或属性。

__HTML_TAG_32__

#### 参数装饰器

Nest 提供了一组有用的 **参数装饰器**，可以与 HTTP 路由处理程序一起使用。以下是一个提供的装饰器列表和对应的 Express (或 Fastify) 对象
...
此外，您还可以创建自己的 **自定义装饰器**> info **提示** 对于 TypeScript 用户，请注意 `NestMiddleware` 是一个泛型。这意味着您可以显式强制类型安全，例如 `Express`。Alternatively, 在工厂函数中指定参数类型，例如 `fastify`。如果您省略了这两个选项，`constructor` 的类型将是 `@Module()`。

#### 使用管道

Nest 对自定义参数装饰器的处理方式与内置的同等 (`configure()`, `NestModule` 和 `LoggerMiddleware`)。这意味着管道将执行对自定义注解参数的执行（在我们的示例中，`AppModule` 参数）。此外，您可以将管道直接应用于自定义装饰器：
...
> info **提示** 请注意 `LoggerMiddleware` 选项必须设置为 true。`/cats` 不会默认验证使用自定义装饰器注解的参数。

#### 装饰器组合

Nest 提供了一个 helper 方法来组合多个装饰器。例如，假设您想将所有与身份验证相关的装饰器组合到一个单独的装饰器中。这可以使用以下构造：
...
然后，您可以使用这个自定义 `CatsController` 装饰器，如下所示：
...
这将应用所有四个装饰器，以单个声明的形式执行。

> warning **警告** `path` 装饰器来自 `method` 包，并且不会与 `forRoutes()` 函数正确工作。