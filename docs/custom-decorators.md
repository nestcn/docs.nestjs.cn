...
<!-- 生成时间: 2026-03-17T04:29:14.767Z -->
<!-- 源文件: content/custom-decorators.md -->

### Custom Route Decorators

Nest 是基于语言特性 called **decorators** 的。Decorators 是很多常用的编程语言中一个知名概念，但是在 JavaScript 世界中它们仍然相对新鲜。为了更好地理解 decorators 的工作，我们建议阅读 __LINK_147__。以下是一个简单的定义：

__HTML_TAG_29__
  ES2016 中的装饰器是一个返回函数的表达式，可以以 target、name 和 property descriptor 作为参数。您可以通过在要装饰的对象上添加 __HTML_TAG_30__@__HTML_TAG_31__ 字符来应用装饰器。Decorators 可以定义为类、方法或属性。

__HTML_TAG_32__

#### Param Decorators

Nest 提供了一组有用的 **paramDecorators**，您可以将其与 HTTP 路由处理程序一起使用。以下是一个提供的装饰器和它们对应的平常 Express (或 Fastify) 对象的列表

__HTML_TAG_33__
  __HTML_TAG_34__
    __HTML_TAG_35__
      __HTML_TAG_36____HTML_TAG_37__@Provider(), @Req()__HTML_TAG_38____HTML_TAG_39__
...
此外，您还可以创建自己的 **custom decorators**。为什么这有用？

在 node.js 世界中，常见做法是将属性附加到 **request> 信息 **提示** TypeScript 用户请注意,__INLINE_CODE_14__ 是一个泛型。这意味着您可以明确强制类型安全，例如 __INLINE_CODE_15__。或者，在工厂函数中指定参数类型，例如 __INLINE_CODE_16__。如果您省略了这两个选项，__INLINE_CODE_17__ 的类型将是 __INLINE_CODE_18__。

#### 使用管道

Nest 对自定义参数装饰器的处理方式与内置装饰器相同（__INLINE_CODE_19__、__INLINE_CODE_20__ 和 __INLINE_CODE_21__）。这意味着管道将对自定义注释参数执行（在我们的示例中是 __INLINE_CODE_22__ 参数）。此外，您可以将管道直接应用于自定义装饰器：
...
> 信息 **提示** 请注意,__INLINE_CODE_23__ 选项必须设置为 true。__INLINE_CODE_24__ 不会默认验证使用自定义装饰器注释的参数。

#### 装饰器组合

Nest 提供了一个帮助方法来组合多个装饰器。例如，假设您想将所有与身份验证相关的装饰器组合到一个单个装饰器中。这可以使用以下构造实现：
...
> 警告 **警告** 来自 `cats` 包的 `@Controller()` 装饰器不可组合且不能正确地与 `@Controller()` 函数一起使用。