...
中间件按特定顺序执行。首先，Nest 运行全局绑定的中间件（例如通过 `app.use` 绑定的中间件），然后运行基于路径确定的 [module bound middleware](/middleware)。中间件按照绑定的顺序依次执行，这与 Express 中中间件的工作方式类似。对于跨不同模块绑定的中间件，绑定到根模块的中间件会首先运行，然后中间件按照模块添加到 imports 数组中的顺序依次运行。
...
> info **提示** 过滤器仅在请求过程中发生任何未捕获异常时执行。已捕获的异常（例如通过 `try/catch` 捕获的异常）不会触发异常过滤器。一旦遇到未捕获的异常，生命周期的其余部分将被忽略，请求直接跳到过滤器。从 [middleware](/middleware#error-handling) 抛出的异常也由异常层处理，但只有**全局**异常过滤器适用，因为中间件在路由处理器被选中之前运行。
