...
TypeScript 的元数据反射系统存在一些限制，例如无法确定类包含哪些属性，或无法识别给定属性是可选的还是必填的。然而，其中一些限制可以在编译时解决。Nest 提供了一个插件来增强 TypeScript 编译过程，从而减少所需的样板代码量。
...
请注意，您的文件名**必须**具有以下后缀之一，插件才会对其进行分析：`['.input.ts', '.args.ts', '.entity.ts', '.model.ts']`（例如，`author.entity.ts`）。如果您使用不同的后缀，可以通过指定 `typeFileNameSuffix` 选项来调整插件的行为（见下文）。
...
虽然对于中型项目来说这不是一个重大问题，但一旦您拥有大量类，代码就会变得冗长且难以维护。
...
该插件基于**抽象语法树**（Abstract Syntax Tree）自动添加适当的装饰器。因此，您不必再为散布在代码中的 `@Field` 装饰器而烦恼。
