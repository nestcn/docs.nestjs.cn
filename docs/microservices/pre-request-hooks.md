...
<!-- 生成时间: 2026-09-13T10:48:05.013Z -->
<!-- 源文件: content/microservices/pre-request-hooks.md -->
<!-- 源哈希: 99f499459dadabfa3fd7d8475cda3ffa -->

### 预请求钩子

预请求钩子是在每个传入的模式处理器调用中，在所有增强器（守卫、拦截器、管道）之前运行的函数。它们是微服务中与 HTTP 中间件等价的概念：用于建立每请求上下文，该上下文必须对守卫及管道其余部分可见。
...
在调用 `app.listen()` 之前，使用 `registerPreRequestHook` 在应用实例上注册钩子。多次调用会按注册顺序累积钩子。

每个钩子接收一个 `ExecutionContext` 和一个 `next` 函数。调用 `next()` 会前进到下一个钩子，或者当没有剩余钩子时，前进到守卫 → 拦截器 → 管道 → 处理器管道。钩子必须返回由 `next()` 产生的 `Observable`。

> warning **警告** 如果钩子不调用 `next()`，处理器将永远不会执行——这与 HTTP 中间件的契约相同。
...
一个常见用例是在守卫运行之前初始化 `AsyncLocalStorage`，使存储在整个管道中可用。
...
由于钩子在守卫之前运行，`als.getStore()` 在任何守卫、拦截器或处理器中返回已初始化的存储：

```typescript title="correlation.guard.ts"
...
可以将 RxJS 操作符应用于 `next()` 返回的 `Observable`，以观察完整的处理器生命周期：
...
> info **提示** `registerPreRequestHook` 仅支持全局——不支持按模式注册。当没有注册钩子时，管道采用与引入此功能之前相同的快速路径。WebSocket 网关支持有意不在范围内。