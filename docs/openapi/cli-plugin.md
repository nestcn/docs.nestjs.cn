...
<!-- 生成时间: 2026-09-13T08:20:18.127Z -->
<!-- 源文件: content/openapi/cli-plugin.md -->
<!-- 源哈希: 14f350655203f9035f97f9117029c464 -->

### CLI 插件

[TypeScript](https://www.typescriptlang.org/docs/handbook/decorators.html) 的元数据反射系统存在一些限制，使得无法确定类的属性组成或识别给定属性是否为可选或必需。然而，其中一些限制可以在编译时解决。Nest 提供了一个插件来增强 TypeScript 编译过程，以减少所需的样板代码量。

> info **提示** 此插件是**可选**的。如果您愿意，可以手动声明所有装饰器，或者只在需要的地方声明特定的装饰器。

#### 概述

Swagger 插件将自动：

- 使用 `@ApiProperty` 注解所有 DTO 属性，除非使用了 `@ApiHideProperty`
...
以前，如果您想提供与 Swagger UI 的交互式体验，您必须复制大量代码，让包知道您的模型/组件应如何在规范中声明。例如，您可以定义一个简单的 `CreateUserDto` 类，如下所示：
...
> info **注意** Swagger 插件将从 TypeScript 类型和 class-validator 装饰器派生 @ApiProperty() 注解。这有助于为生成的 Swagger UI 文档清晰地描述您的 API。但是，运行时的验证仍将由 class-validator 装饰器处理。因此，需要继续使用诸如 `IsEmail()`、`IsNumber()` 等验证器。

因此，如果您打算依赖自动注解来生成文档，并且仍然希望进行运行时验证，那么 class-validator 装饰器仍然是必需的。

> info **提示** 在 DTO 中使用 [mapped types utilities](/openapi/mapped-types)（如 `PartialType`）时，请从 `@nestjs/swagger` 而不是 `@nestjs/mapped-types` 导入它们，以便插件获取模式。

插件根据**抽象语法树**动态添加适当的装饰器。因此，您不必再为散布在代码中的 `@ApiProperty` 装饰器而烦恼。
...
您必须同时复制描述和示例值。启用 `introspectComments` 后，CLI 插件可以提取这些注释，并自动为属性提供描述（以及示例，如果定义了的话）。现在，上述属性可以简单地声明如下：
...
有 `dtoKeyOfComment` 和 `controllerKeyOfComment` 插件选项可用于自定义插件分别如何为 `ApiProperty` 和 `ApiOperation` 装饰器赋值。请参见下面的示例：
...
> info **提示** 对于模型，同样的逻辑适用，但改用 `ApiProperty` 装饰器。
...
从 `@nestjs/swagger` v12 开始，`@param` 标签也会被拾取，因此路由参数描述可以来自同一个文档注释：
...
插件为可选的 `@Query()` 参数添加一个 `description` 到它已经生成的 `@ApiQuery()`，为文档化的必需查询参数生成 `@ApiQuery({ name, description &#125;)`，并为文档化的 `@Param()` 参数生成 `@ApiParam({ name, description &#125;)`。

描述通过**变量名**匹配，这正是 `@param` 所记录的 - 因此 `@Query('order_by') orderBy: string` 被记录为 `@param orderBy ...`，而不是 `@param order_by ...`。显式的 `@ApiQuery()` / `@ApiParam()` 装饰器始终具有优先权，永远不会被覆盖。
...
确保在更新插件选项时删除 `/dist` 文件夹并重新构建您的应用程序。
...
对于标准设置（非 monorepo），要使用 CLI 插件与 SWC 构建器，您需要启用类型检查，如 [here](/recipes/swc#类型检查) 所述。
...
如果 `jest` 似乎没有获取您的配置更改，可能是 Jest 已经**缓存**了构建结果。要应用新配置，您需要清除 Jest 的缓存目录。
