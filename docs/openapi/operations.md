...
`parent` 选项按名称引用另一个标签，而 `kind` 是一个自由格式的机器可读字符串，提示标签应如何使用——通常是 `nav`、`badge` 或 `audience`。

> warning **警告** `parent` 和 `kind` 字段属于 OpenAPI 3.2 Tag Object。您必须调用 `setOpenAPIVersion('3.2.0')`，否则生成的文档仍会声明 `openapi: 3.0.0`，严格的验证器将拒绝这些字段。层次结构字段只能通过 `DocumentBuilder.addTag()` 定义；在 `@ApiTags()` 装饰器上设置它们无效。
...
与其为每个端点或控制器单独定义响应，您可以使用 `DocumentBuilder` 类为所有端点定义全局响应。当您想为应用程序中的所有端点定义全局响应（例如，对于像 `401 Unauthorized` 或 `500 Internal Server Error` 这样的错误）时，这种方法很有用。
...
您可以使用 `@ApiBody` 装饰器以及 `@ApiConsumes()` 为特定方法启用文件上传。以下是使用 [File Upload](/techniques/file-upload) 技术的完整示例：
...
我们跳过装饰 `results`，因为稍后我们将为其提供原始定义。现在，让我们定义另一个 DTO，并将其命名为例如 `CatDto`，如下所示：
...
有了这些，我们可以如下定义 `PaginatedDto<CatDto>` 响应：
...
如果您现在运行 Swagger，此特定端点生成的 `swagger.json` 应定义以下响应：
...
为了确保 `SwaggerModule` 将为我们的模型生成定义，我们必须将其添加为额外模型，就像我们之前在控制器中使用 `PaginatedDto` 所做的那样。
