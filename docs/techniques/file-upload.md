...
<!-- 生成时间: 2026-09-13T08:54:31.023Z -->
<!-- 源文件: content/techniques/file-upload.md -->
<!-- 源哈希: d17ef9678392b311f03139069b09b59b -->

### 文件上传

为了处理文件上传，Nest 提供了一个基于 Express 的 [multer](https://github.com/expressjs/multer) 中间件包的内置模块。Multer 处理以 `multipart/form-data` 格式发布的数据，该格式主要用于通过 HTTP `POST` 请求上传文件。该模块完全可配置，您可以根据应用程序需求调整其行为。
...
> warning **警告** `FileInterceptor()` 可能与 Google Firebase 等第三方云提供商不兼容。

#### 文件验证

通常，验证传入的文件元数据（如文件大小或文件 MIME 类型）是很有用的。为此，您可以创建自己的 [Pipe](/pipes) 并将其绑定到带有 `UploadedFile` 装饰器的参数上。下面的示例演示了如何实现基本的文件大小验证器管道：
...
<table>
  <tr>
    ...
    <td>在 <b>任何</b>验证器失败时抛出的 HTTP 状态码。默认值为 <code>400</code>（错误请求）</td>
  </tr>
  <tr>
    <td><code>exceptionFactory</code></td>
    <td>一个工厂，接收错误消息并返回错误。</td>
  </tr>
</table>
...
`FileValidator` 是一个常规类，可以访问文件对象并根据客户端提供的选项进行验证。Nest 有两个内置的 `FileValidator` 实现，您可以在项目中使用：

- `MaxFileSizeValidator` - 检查给定文件的大小是否小于提供的值（以 `bytes` 为单位）
- `FileTypeValidator` - 检查给定文件的 MIME 类型是否匹配给定的字符串或 RegExp。默认情况下，使用文件内容验证 MIME 类型 [magic number](https://www.ibm.com/support/pages/what-magic-number)
...
> info **提示** 如果验证器数量大幅增加或其选项使文件混乱，您可以在单独的文件中定义此数组，并在此处作为命名常量导入，如 `fileValidators`。

最后，您可以使用特殊的 `ParseFilePipeBuilder` 类来组合和构造验证器。通过如下所示的使用，您可以避免手动实例化每个验证器，直接传递其选项：
...
要上传文件数组（使用单个字段名称标识），请使用 `FilesInterceptor()` 装饰器（注意装饰器名称中的复数 **Files**）。此装饰器接受三个参数：
...
要上传多个文件（所有文件使用不同的字段名键），请使用 `FileFieldsInterceptor()` 装饰器。此装饰器接受两个参数：
...
您可以如上所述在文件拦截器中指定 multer 选项。要设置默认选项，您可以在导入 `MulterModule` 时调用静态的 `register()` 方法，传入支持的选项。您可以使用 [here](https://github.com/expressjs/multer#multeropts) 中列出的所有选项。
...
上面的构造在 `MulterModule` 内部实例化 `MulterConfigService`，使用它来创建所需的选项对象。请注意，在此示例中，`MulterConfigService` 必须实现 `MulterOptionsFactory` 接口，如下所示。`MulterModule` 将在所提供类的实例化对象上调用 `createMulterOptions()` 方法。
...
当您想为工厂函数或类构造函数提供额外的依赖时，这非常有用。

#### 示例

一个可用的示例可在 [here](https://github.com/nestjs/nest/tree/master/sample/29-file-upload) 中找到。