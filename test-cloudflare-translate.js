...
async function testTranslation() {
  console.log('🧪 Testing Cloudflare Workers AI translation functionality...\n');
  ...
  // 如果配置了 Cloudflare 凭据，添加 AI 翻译测试
  if (process.env.CLOUDFLARE_API_TOKEN && process.env.CLOUDFLARE_ACCOUNT_ID) {
    testConfigs.push({
      name: 'Cloudflare Workers AI 翻译',
      options: { 
        contentDir: testContentDir, 
        docsDir: testDocsDir + '-cloudflare',
        useAI: true,
        verbose: true 
      }
    });
  } else {
    console.log('⚠️ Cloudflare API 凭据未配置，跳过 AI 翻译测试');
    console.log('   请设置环境变量:');
  ...
  try {
    ...
    fs.rmSync(testDocsDir + '-cloudflare', { recursive: true, force: true });
    console.log('✅ 清理完成');
  } catch (error) {
